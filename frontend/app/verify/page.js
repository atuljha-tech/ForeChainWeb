...
export default function Verify() {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${themeStyles.background} transition-colors duration-300`}>
      ...
      <main className="flex-grow p-3 sm:p-4 lg:p-6 max-w-4xl mx-auto w-full">
        {/* Page Header */}
        <div className="mb-4 sm:mb-6 lg:mb-8 text-center">
          <h1 className={`text-xl sm:text-2xl lg:text-3xl xl:text-4xl font-bold mb-2 sm:mb-3 font-mono ${themeStyles.textPrimary} ${isDarkMode ? 'glow-text' : ''}`}>
            🔍 CRYPTOGRAPHIC_VERIFICATION
          </h1>
          <p className={`font-mono text-xs sm:text-sm lg:text-base ${themeStyles.textSecondary} mb-1 sm:mb-2`}>
            // SHA-256 Hash Validation • Blockchain Integrity Check
          </p>
          <div className={`mt-1 sm:mt-2 font-mono text-xs ${themeStyles.textMuted}`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 lg:gap-4 mb-4 sm:mb-6">
          <div className={`${themeStyles.cardBackground} border ${isDarkMode ? 'border-blue-700' : 'border-blue-300'} rounded-lg p-2 sm:p-3 text-center`}>
            <div className={`text-base sm:text-lg ${themeStyles.textPrimary}`}>📊</div>
            <div className={`font-mono text-sm sm:text-base ${themeStyles.textPrimary}`}>{blockchainReports.length}</div>
            <div className={`font-mono text-xs ${themeStyles.textMuted}`}>ON CHAIN</div>
          </div>
          <div className={`${themeStyles.cardBackground} border ${isDarkMode ? 'border-green-700' : 'border-green-300'} rounded-lg p-2 sm:p-3 text-center`}>
            <div className={`text-base sm:text-lg ${isDarkMode ? 'text-green-400' : 'text-green-500'}`}>✅</div>
            <div className={`font-mono text-sm sm:text-base ${isDarkMode ? 'text-green-300' : 'text-green-600'}`}>
          ...
          <div className={`${themeStyles.cardBackground} border ${isDarkMode ? 'border-purple-700' : 'border-purple-300'} rounded-lg p-2 sm:p-3 text-center`}>
            <div className={`text-base sm:text-lg ${isDarkMode ? 'text-purple-400' : 'text-purple-500'}`}>⛓️</div>
            <div className={`font-mono text-xs sm:text-sm ${isDarkMode ? 'text-purple-300' : 'text-purple-600'}`}>Ethereum</div>
            <div className={`font-mono text-xs ${isDarkMode ? 'text-purple-500' : 'text-purple-500'}`}>NETWORK</div>
          </div>
          <div className={`${themeStyles.cardBackground} border ${isDarkMode ? 'border-cyan-700' : 'border-cyan-300'} rounded-lg p-2 sm:p-3 text-center`}>
            <div className={`text-base sm:text-lg ${isDarkMode ? 'text-cyan-400' : 'text-cyan-500'}`}>🔐</div>
            <div className={`font-mono text-xs sm:text-sm ${isDarkMode ? 'text-cyan-300' : 'text-cyan-600'}`}>SHA-256</div>
            <div className={`font-mono text-xs ${isDarkMode ? 'text-cyan-500' : 'text-cyan-500'}`}>ALGORITHM</div>
          </div>
        </div>

        {/* Main Verification Card */}
        <div className={`${themeStyles.cardBackground} border-2 ${themeStyles.cardBorder} rounded-xl p-3 sm:p-4 lg:p-6 xl:p-8 transition-all duration-300 hover:shadow-lg ${isDarkMode ? 'hover:shadow-blue-500/20' : 'hover:shadow-blue-500/10'}`}>
          ...
          <div className="mb-4 sm:mb-6">
            <label className={`block font-mono text-xs sm:text-sm mb-2 sm:mb-4 uppercase tracking-wider ${themeStyles.textSecondary}`}>
            ...
            <div className={`border-2 border-dashed rounded-lg p-4 sm:p-6 text-center transition-all duration-300 ${themeStyles.uploadArea}`}>
              <div className={`mb-2 sm:mb-4 ${themeStyles.textMuted}`}>
              ...
              <label 
                htmlFor="file-upload"
                className={`inline-block px-4 py-2 sm:px-6 sm:py-3 font-mono text-xs sm:text-sm rounded border transition-all duration-300 cursor-pointer ${
                  themeStyles.buttonPrimary
                } ${themeStyles.buttonHover} hover:shadow-lg mb-3 sm:mb-4`}
              >
                {file ? 'CHANGE_FILE' : 'SELECT_FILE'}
              </label>
              
              <p className={`font-mono text-xs sm:text-sm mb-1 sm:mb-2 ${themeStyles.textPrimary}`}>
            ...
            {/* File Info - Improved Mobile Layout */}
            {file && (
              <div className={`mt-3 sm:mt-4 rounded-lg p-3 sm:p-4 border ${themeStyles.fileInfo}`}>
                <h4 className={`font-mono text-xs sm:text-sm mb-2 sm:mb-3 uppercase tracking-wider ${themeStyles.textSecondary}`}>
                  FILE_DETAILS
                </h4>
                <div className="grid grid-cols-1 xs:grid-cols-2 gap-2 sm:gap-3 lg:gap-4">
                  <div>
                    ...
                    <p className={`truncate font-mono text-xs sm:text-sm ${themeStyles.textPrimary}`}>{file.name}</p>
                  </div>
                  <div>
                    <span className={`${themeStyles.textMuted} font-mono text-xs`}>Size:</span>
                    <p className={`font-mono text-xs sm:text-sm ${themeStyles.textPrimary}`}>{(file.size / 1024).toFixed(2)} KB</p>
                  </div>
                  <div>
                    <span className={`${themeStyles.textMuted} font-mono text-xs`}>Type:</span>
                    <p className={`font-mono text-xs sm:text-sm ${themeStyles.textPrimary}`}>{file.type || 'Unknown'}</p>
                  </div>
                  <div>
                    <span className={`${themeStyles.textMuted} font-mono text-xs`}>Modified:</span>
                    <p className={`font-mono text-xs sm:text-sm ${themeStyles.textPrimary}`}>{new Date(file.lastModified).toLocaleDateString()}</p>
                  </div>
                </div>
              </div>
            )}
          ...
          {/* Verification Controls - Stack on Mobile */}
          <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
            <button 
              onClick={handleVerify}
              disabled={!file || isVerifying}
              className={`py-2 sm:py-3 px-4 sm:px-6 font-mono text-xs sm:text-sm rounded border transition-all duration-300 ${
                !file || isVerifying
            ...
            <button 
              onClick={handleClear}
              className={`py-2 sm:py-3 px-4 sm:px-6 font-mono text-xs sm:text-sm rounded border transition-all duration-300 ${themeStyles.buttonSecondary}`}
            >
              🗑️ CLEAR
            </button>
          </div>
          ...
          {/* Enhanced Verification Details - Improved Mobile Layout */}
          {verificationData && (
            <div className={`rounded-lg p-3 sm:p-4 border mb-4 sm:mb-6 ${themeStyles.detailsPanel}`}>
              <h4 className={`font-mono text-xs sm:text-sm mb-3 sm:mb-4 uppercase tracking-wider ${themeStyles.textSecondary}`}>
                CRYPTOGRAPHIC_VERIFICATION_REPORT
              </h4>
              
              <div className="space-y-2 sm:space-y-3 font-mono text-xs sm:text-sm">
                {/* Verification Status */}
                <div className="flex flex-col xs:flex-row xs:justify-between xs:items-center p-2 sm:p-3 rounded border bg-opacity-20 gap-2 xs:gap-0">
                  <span className={themeStyles.textMuted}>Verification Status:</span>
                  <span className={`px-2 py-1 sm:px-3 sm:py-1 rounded text-xs font-bold ${
                ...
                {/* Hash Comparison - Stack on Mobile */}
                ...
                {/* Blockchain Info - Stack on Mobile */}
                {verificationData.isOnChain && (
                  <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 sm:gap-4 pt-2 sm:pt-3 border-t border-opacity-30">
                    <div>
                      <span className={themeStyles.textMuted}>Uploaded By:</span>
                      <p className={`${themeStyles.textPrimary} break-words`}>{verificationData.uploader}</p>
                    </div>
                    <div>
                ...
                {/* Security Badge */}
                <div className={`mt-3 sm:mt-4 p-2 sm:p-3 rounded border text-center ${
                  verificationData.matches 
          ...
          {/* System Status */}
          <div className="pt-3 sm:pt-4 border-t border-opacity-30">
